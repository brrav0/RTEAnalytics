...
wind <- cbind(DataSet$Date,DataSet$Time,DataSet$Wind)
Year <- year(as.Date(DataSet$Date,format="%m/%d/%Y")) 
Month <- month(as.Date(DataSet$Date,format="%m/%d/%Y")) 
Winter_month <-subset(DataSet$Date,Month==12,1:2)
Spring_month <-subset(DataSet$Date,Month==3:5)
Summer_month <-subset(DataSet$Date,Month==6:8)
Winter_month <-subset(DataSet$Date,Month==9:11)
