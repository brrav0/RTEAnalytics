...
#list_of_sources=colnames(c(DataSet$Fuel,DataSet$Coal, DataSet$Gas, DataSet$Nuclear, DataSet$Wind, DataSet$Solar, DataSet$Hydro))
#energyMixYear <-  group_by(DataSet, Year) %>% summarise(Wind = sum(Wind), Coal = sum(Coal))
#plot(gvisSteppedAreaChart(energyMixYear,xvar = "Year", yvar = c("Wind", "Coal"), options=list(isStacked=TRUE)))
```
