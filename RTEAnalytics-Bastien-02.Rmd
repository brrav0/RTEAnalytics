...
print(gvisLineChart(DataSet, xvar = "Date", yvar = c("Consumption"), options=list(isStacked=TRUE,width = 1000, height = 500, vAxis="{format:'#,###GWh'}")), 'chart')
```

We want to see the correlation between the forecast and the consumption. The correlation amount to `r cor(Consumption,Forecast)`. We can conclude that the forecast is of high quality.
...
#data below works be superpatient
print(gvisLineChart(DataSet, xvar = "Date", yvar = c("Difference"), options=list(isStacked=TRUE,width = 1000, height = 500, vAxis="{format:'#,###GWh'}")), 'chart')
```
